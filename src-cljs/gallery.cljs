...
(comment
   ...
   (doseq [{:keys [name status]} response]
     (if (= "ok" status)
       (-> (by-id name)
           ...
           (.-parentNode)
           (dom/removeNode))
       (.append errors (str "<li>failed to remove " name ": " status "</i>"))))
   (let [error-string (str "<ul>" (.toString errors) "</ul>")]
     (if-not (empty? error-string)
       (append! (by-id "error") error-string))))
 ...
 (defn ^:export init []
   (listen! (sel "#delete") :click (js/alert "Clicked delete"))))
